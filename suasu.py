...
#### STEAL PROFILE ####
                            ...
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                                ...
                                cl.sendMessage(kirim, str(ret_))
                                #cl.sendContact(kirim, contact.mid)

                        elif PuyText.lower().startswith("psteal 6 "):
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                                names = re.findall(r'@(\w+)', text)
                                mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                                mentionees = mention['MENTIONEES']
...
### SC ###
                        ...
                        elif PuyText.lower().startswith('filmz'):
                            try:
                                sep = PuyText.split(" ")
                                search = PuyText.replace(sep[0] + " ","")
                                r = requests.get("https://v1.mazterin.com/cari_film.php?key=6d363479c97439b921ad2bcba054992d8eda9a0c&q={}".format(search))
                                data = r.json()
                                a=" 「 Fun 」\nType: Films\n"
                                a+="\nBahasa "+str(data["result"]["bahasa"])
                                a+="\nJudul: "+str(data["result"]["judul"])
                                a+="\nTahun: "+str(data["result"]["tahun"])
                                a+="\nDurasi: "+str(data["result"]["durasi"])
                                a+="\nGenre: "+str(data["result"]["genre"])
                                cl.sendMessage(kirim,a)
                            except Exception as e:
                                cl.sendMessage(kirim, str(e))

                        elif PuyText.lower().startswith("pilem "):
                            try:
                                sep = PuyText.split(" ")
                                search = PuyText.replace(sep[0] + " ","")
                                r = requests.get("http://rest.farzain.com/api/film.php?id={}&apikey=YAfYs3tyIJJF91Ruc7iw8eW7E".format(search))
                                data = r.json()
                                a=" 「 Fun 」\nType: Drama Korea\n"
                                a+="\n "+str(data["Title"])
                                a+="\n "+str(data["Year"])
                            ...
                            try:
                                sep = PuyText.split(" ")
                                search = PuyText.replace(sep[0] + " ","")
                                r = requests.get("https://api.eater.pw/danbooru/{}".format(search))
                                data = r.json()
                                a=" 「 Fun 」\nType: Danbooru\n"
                                a+="\nInfo: "+str(data["result"]["info"])
                                #a+="\nImg: "+str(data["result"]["img"])
                                #a+="\n "+str(data["result"]["info"])
                                cl.sendMessage(kirim,a)
                                cl.sendImageWithURL(kirim, "https://api.eater.pw/danbooru/{}".format(search))
                            except Exception as e:
                                cl.sendMessage(kirim, str(e))

                        elif PuyText.lower().startswith("suasu "):
                            no = time.time()
                            cl.sendText("uac8e3eaf1eb2a55770bf10c3b2357c33", ' ')
                            elapsed_time = time.time() - no
                            sep = PuyText.split(" ")
                            ...
                            with requests.session() as s:
                                ...
                                if data["status"] == True:
                                    ...
                                    cl.sendImageWithURL(kirim, str(path))
                                    cl.sendMessage(kirim,"[Image Result]\nGot image in %s seconds" %(elapsed_time))
                                else:
                                    cl.sendMessage(kirim, "Hasil pencarian tidak ditemukan")

                        elif PuyText.lower().startswith("samehadaku"):
                            sep = PuyText.split(" ")
                            kudanil = PuyText.replace(sep[0] + " ","")
                            with requests.session() as s:
                                s.headers['user-agent'] = 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36'
                                r = s.get("https://api.farzain.com/samehadaku.php?id={}&apikey=VOPMovDez0KTrKsqApvheRdvN".format(urllib.parse.quote(kudanil)))
                                data = r.text
                                data = json.loads(data)
                                ret_ = "-=SAMEHADAKU=-"
                                no = 1
                                for kambing in data:
                                    ...
                                    aaa = kambing['date']
                                    hfe = kambing['img']
                                    ret_ += "\n{}. Judul: {}\n Url: {}\n Date: {}\n Image: {}".format(str(no), str(ttl), str(iii), str(aaa), str(hfe))
                                    no += 1
                                cl.sendMessage(kirim, str(ret_))

                        elif PuyText.lower().startswith("axax"):
                            try:
                                sep = PuyText.split(" ")
                                kudanil = PuyText.replace(sep[0] + " ","")
                                with requests.session() as s:
                                    s.headers['user-agent'] = 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36'
                                    r = s.get("http://api.farzain.com/samehadaku.php?id={}&apikey=VOPMovDez0KTrKsqApvheRdvN".format(urllib.parse.quote(kudanil)))
                                    data = r.text
                                    data = json.loads(data)
                                    ret_ = "-=SAMEHADAKU=-"
                                    no = 1
                        ...
                        elif PuyText.lower().startswith('screenshot'):
                            sep = PuyText.split(" ")
                            puy = PuyText.replace(sep[0] + " ","")
                            cl.sendImageWithURL(kirim, 'http://api.screenshotmachine.com/?key=1ae4ff&dimension=1024x768&format=jpg&url='+puy)

                        elif PuyText.lower().startswith("love"):
                          if user in Owner:
                            sep = text.split(" ")
                            ...
                            cl.sendMessage(kirim, "Hasil {} dengan {} adalah {}%".format(name1,name2,percentage))
                        ...
                        elif PuyText.lower().startswith('igpost'):
                            try:
                                #sep = PuyText.split(" ")
                                search = PuyText.replace("igpost ","")
                                r = requests.get("https://rest.farzain.com/api/ig_post.php?id={}?_a=1&apikey=YAfYs3tyIJJF91Ruc7iw8eW7E".format(search))
                                data = r.json()
                                a=" 「 Fun 」\nType: Instagram Post\n"
                                a+="\nDisukai : "+str(data["like"])
                                a+="\nKomentar : "+str(data["comment"])
                                #a+="\nPersen : "+str(data["percentage"])
                                #a+="\nKeterangan : "+str(data["result"])
                                cl.sendMessage(kirim,a)
                            except Exception as e:
